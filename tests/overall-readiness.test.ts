...
describe('overall delivery readiness',()=>{
  it('quantifies why a composite remains gated',()=>{
    ...
    expect(r.periods).toEqual(['July 2026']);expect(r.exactCandidateOverlap).toBe(19);expect(r.validThreeSubjectCandidates).toBe(19);
    expect(r.validCoveragePercent).toBe(100);expect(r.zeroToiletAndVehicle).toBe(18);expect(r.nonDegenerateCandidates).toBe(1);
