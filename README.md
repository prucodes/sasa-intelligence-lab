...
- **SAMPLE** uses authenticated governed JSON exports retrieved through the signed-in AI Living Labs API Playground. The retained evidence now covers 44 complete historical exports plus six large authenticated routes reconciled from 42,502 pages and 4,249,741 raw rows, repeats included. The current catalogue contains 42 authorized routes, all 42 with a retained response or export; eight older keys remain visible as separate historical vintages because the current catalogue no longer lists them. Complete payloads do not remove the need for a reviewed crosswalk or same-year outcome evidence, so all source profiles remain unscored.
- **Large-route retention** is recorded in `data/large-retention-audit.json`. The page validator requires contiguous offsets, stable upstream totals, and a final page with no next token before a route is marked retained. Large raw pages stay outside the browser bundle; the application uses their validated rollups and provenance summaries.
- **Historical vintages** remain separate from current routes. Revised LGD-enriched routes replace older route names only where the source explicitly supplies the replacement; the app does not silently merge similarly named exports.
...
The current dataset-use reconciliation covers the retained catalogue and its six large routes: retained datasets power primary analytics or supporting programme/history/evidence-breadth views, and no complete retained dataset is left without a visible role. The exact register is rendered in **Data Readiness → Catalogue**.
