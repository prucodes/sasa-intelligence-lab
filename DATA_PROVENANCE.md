...
On 2026-09-14 the missing rows were recovered with filtered queries small enough to fit one page, and merged into both retained copies of six tables: legacy waste clearance (3 rows), compost pits (3), soak pits (3), magic drains (2), IHHL new identification (2) and sewage (6). The green programme table, served under three keys, had 5 rows recovered in its 2026-09-10 copy; its only earlier copy is a different 244-row vintage. Every other row is unchanged, each file records the repair under `retentionRepair`, and the raw filtered responses are kept in `data/retention-repairs/2026-09-14/`.
