import {getRuralMovement,observationText,type RateBasis} from './rural-movement';
...
export function getGapCapabilities(basis:RateBasis='all-days') {
  ...
  return [
    ...
    {id:'movement',label:'May–August trends',figure:{value:signed(shift),unit:`percentage points across four months, on ${observationText(movement.cohort.observations)} matched observations${movement.cohort.observations.sameEachMonth?'':' a month'}`},finding:`A modest statewide shift, and ${movement.declining.length} districts that decline at every step.`,available:movement.cohort.pairs>0,scope:movement.cohort.observations.sameEachMonth?`${observationText(movement.cohort.observations)} common GP-day observations`:`${observationText(movement.cohort.observations)} GP-day observations, a different subset each month`,boundary:'Reported first-week trends across four months; not programme effects.'},
