...
export function RadarNames({points,bounds={left:78,right:780,top:96,bottom:455},size=10,reserved=[]}:{points:NamedPoint[];bounds?:Bounds;size?:number;reserved?:Box[]}){
 ...
 return <g aria-label="Named ULB examples" style={{pointerEvents:'none'}}>{points.map(p=>{
  ...
  return <g key={p.key}><line x1={p.x} y1={p.y} x2={lx} y2={ly-4} stroke="currentColor" opacity=".35"/><text x={lx} y={ly} textLength={p.name.length*size*.64>width?width:undefined} lengthAdjust="spacingAndGlyphs" style={{fontSize:size,fontWeight:650,fill:'var(--ink)',paintOrder:'stroke',stroke:'var(--surface,#fff)',strokeWidth:4,strokeLinejoin:'round'}}>{p.name}</text></g>;
 })}</g>;
}
