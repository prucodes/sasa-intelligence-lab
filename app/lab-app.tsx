    ...
    <div className="presenter-rural-layout">
      <div className="presenter-rural-plot">
        ...
        <p className="presenter-rural-basis">
          Every seven-day window holds one Sunday, which reports almost no collection. Leaving that Sunday out, the rest of the cohort reads{' '}
...
function ReadinessCommandCenter({ readiness }: { readiness: ReturnType<ReturnType<typeof createProvider>['getReadiness']> }) {
  ...
  return <section className="screen-feature sf-readiness" aria-label="Evidence activation pipeline">
    ...
    <div className="sf-readiness-exceptions" aria-label="Current source synchronization"><div><b>{readinessCatalogueStats.retainedCurrentRoutes}</b><span>current route responses<br/>retained for review</span></div><div><b>{readinessCatalogueStats.platformAvailable-readinessCatalogueStats.retainedCurrentRoutes}</b><span>current routes<br/>not refreshed in this pass</span></div><div><b>{readinessCatalogueStats.notInCurrentCatalogue}</b><span>historical keys<br/>absent from current catalogue</span></div><p>All six large routes are now reconciled from authenticated export and paginated evidence ({formatValue(readinessCatalogueStats.largeRetentionRows)} rows across {formatValue(readinessCatalogueStats.largeRetentionPages)} pages).<br/>Historical keys absent from the current catalogue remain visible as separate vintages; they are not silently merged into current routes.<br/><a href="/reconciliation?mode=governed&view=revisions">Inspect the {revisedRoutes} revised routes →</a></p></div>
