...
export function RuralMovement({compact=false,findingStated=compact,href='/gap-radar?mode=governed&view=movement',basis:controlledBasis,onBasisChange}:{compact?:boolean;findingStated?:boolean;href?:string;basis?:RateBasis;onBasisChange?:(basis:RateBasis)=>void}) {
 ...
 <div className="ew-controls"><label>Days counted<select aria-label="Rural collection day basis" value={basis} onChange={e=>setBasis(e.target.value as RateBasis)}><option value="all-days">All seven days · includes the Sunday</option><option value="working-days">Excluding Sundays and second Saturdays</option></select></label></div>
 <details className="ew-caption"><summary>How the day selection affects these rates</summary><p>{basis==='all-days'
  ?`Each seven-day window contains one Sunday, and those Sundays report ${droppedRange} across the four months. They are recurring low-reporting days, and the cause is not confirmed. Every month carries exactly one, so the periods stay comparable, but the level sits below the rate on the other days.`
  :`Sundays and second Saturdays are excluded: ${data.excludedDays.map(e=>e.days.map(d=>`${d.date} (${d.reason})`).join(', ')).filter(Boolean).join(' · ')}. Rates are read over the remaining days, so each month uses a slightly different part of the same cohort.`}</p></details>
 <div className="movement-geography-layout"><div className="movement-series-story"><span className="ew-kicker">{district||'All matched districts'} · May–August 2026</span>{(!compact||district)&&<><strong className="movement-series-value">{signed(change)}</strong><p>percentage points, from {pct(points[0].collectionRate)} in {month(points[0].period)} to {pct(points.at(-1)!.collectionRate)} in {month(points.at(-1)!.period)}</p></>}<small>{observationText(selected?.observations??data.cohort.observations)} matched GP-day observations · first seven days · {basis==='all-days'?'all seven days':'excluding Sundays and second Saturdays'}</small>
 ...
 <p className="ew-caption">Yes ÷ valid Yes/No collection reports, {basis==='all-days'?'all seven days':'excluding Sundays and second Saturdays'}. Full 0–100% scale; {basis==='all-days'?'the same GP and day-of-month observations are used in every month.':'each month drops the Sunday in its first week, so the months use slightly different parts of the same cohort.'}</p></div>
 <DistrictMap rows={data.districts.map(d=>({district:d.district,value:d.changePercentagePoints,detail:`${observationText(d.observations)} matched observations · ${d.points.map(p=>pct(p.collectionRate)).join(' → ')}`}))} selected={district} onSelect={setDistrict} title="May → August change by district" unit="percentage points" diverging/>
 </div>
 <div className="movement-ledger"><span><b>{observationText(data.cohort.observations)}</b> {data.cohort.observations.sameEachMonth?'common GP-day observations':'GP-day observations, a different subset each month'}</span><span><b>{data.declining.length}</b> districts decrease at every step</span><span><b>{data.rising.length}</b> increase at every step</span><span><b>{data.periods.length}</b> validated retained months</span></div>
 ...
 <p className="ew-notice">These are reported first-week trends, not full-month service ratings or established programme effects. Day-of-month pairing holds the weekday mix roughly constant rather than eliminating it, and the basis without Sundays and second Saturdays excludes them by calendar rather than by any published holiday list. Stored zero/No responses may reflect reporting behaviour.</p>
