...
export function RuralMovement({compact=false,href='/gap-radar?mode=governed&view=movement'}:{compact?:boolean;href?:string}) {
 ...
 <header className="ew-heading"><div><span className="ew-kicker">Rural collection · May → August 2026</span><h2>A modest statewide shift.<br/><em>{data.declining.length} districts decline at every step.</em></h2><p>First seven days of each month · one common cohort · reported collection activity</p></div></header>
 ...
 <div className="movement-geography-layout"><div className="movement-series-story"><span className="ew-kicker">{district||'All matched districts'}</span><strong className="movement-series-value">{pp(change)}</strong><p>change in reported collection activity · May → August</p><small>{n(selected?.pairs??data.cohort.pairs)} matched GP-day observations in every month</small>
 ...
 <DistrictMap rows={data.districts.map(d=>({district:d.district,value:d.changePercentagePoints,detail:`${n(d.pairs)} matched observations · ${d.points.map(p=>pct(p.collectionRate)).join(' → ')}`}))} selected={district} onSelect={setDistrict} title="May → August change by district" unit="pp" diverging/>
 </div>
