...
export function UlbRadar(){
  const narrow=useCompactChart(),left=narrow?85:70,right=narrow?372:680,top=70,bottom=narrow?410:430;
  ...
  const x=(v:number)=>left+v/ceiling*(right-left),y=(rate:number)=>bottom-rate/100*(bottom-top);
  ...
  return <section className="ulb-radar" aria-label="ULB delivery comparison">
    ...
    <div className="ur-stage"><div className="ur-chart"><svg className={narrow?'compact-delivery':undefined} viewBox={narrow?'0 0 400 480':'0 0 740 510'} role="group" aria-label="ULB workload versus reported completion">
      ...
      <line x1={left} x2={right} y1={y(50)} y2={y(50)} className="ur-reference"/>{data.median!==null&&<line x1={x(data.median)} x2={x(data.median)} y1={top} y2={bottom} className="ur-reference"/>}
      {!narrow&&<g><text x="78" y="24" className="ur-quadrant">Smaller workload</text><text x="674" y="24" textAnchor="end" className="ur-quadrant">Larger workload</text><text x="674" y="51" textAnchor="end" className="ur-zone-label">Upper half: at least half completed</text><text x="674" y="274" textAnchor="end" className="ur-zone-label">Lower half: more than half still to deliver</text></g>}
      <text x={(left+right)/2} y={narrow?468:494} textAnchor="middle" className="ur-axis">{narrow?'Reported workload':`Reported workload · ${data.basisLabel}`}</text><text transform={narrow?'translate(15 240) rotate(-90)':'translate(17 250) rotate(-90)'} textAnchor="middle" className="ur-axis">{narrow?'Reported completion (%)':`${data.completedLabel} / ${data.basisLabel} (%)`}</text>
      {groups.map((group,index)=>{const active=group.points.some(p=>p.key===selected),paired=group.points.some(p=>p.key===comparison);return <g key={index} tabIndex={0} role="button" aria-label={`${group.points.map(p=>p.ulb).join(', ')}: ${rateText(group.rate)}% complete, ${n(group.basis)} ${data.basisLabel}${group.points.length>1?`, ${group.points.length} overlapping ULBs`:''}`} aria-pressed={active} className={`ur-dot ${active?'is-selected':''} ${paired?'is-compared':''}`} onClick={()=>setSelected(group.points[0].key)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();setSelected(group.points[0].key);}}}><circle cx={x(group.basis)} cy={y(group.rate)} r="15" fill="transparent"/><circle cx={x(group.basis)} cy={y(group.rate)} r={active||paired?9:group.points.length>1?7:5} className={group.rate>=50?'ur-dot-high':'ur-dot-low'}/>{group.points.length>1&&<text x={x(group.basis)} y={y(group.rate)+3} textAnchor="middle" className="ur-cluster-count">{group.points.length}</text>}{active&&chosen&&!narrow&&<text x={x(group.basis)+(group.basis>ceiling*.75?-12:12)} y={y(group.rate)-16} textAnchor={group.basis>ceiling*.75?'end':'start'} className="ur-selected-label">{chosen.ulb}</text>}<title>{group.points.map(p=>p.ulb).join(', ')}</title></g>;})}
    </svg>{narrow&&<p className="ur-chart-note"><b>{data.completedLabel} ÷ {data.basisLabel}</b><br/>Green: at least half completed. Amber: more than half remains.{chosen&&<><br/>Selected: <b>{chosen.ulb}</b> · {rateText(chosen.rate)}%</>}</p>}<p className="ur-chart-note">Vertical line: cohort median workload ({n(data.median??0)}). Horizontal line: 50% completed, a reading guide, not an official performance threshold. Numbered dots contain overlapping ULBs; use the selector to inspect every member. References stay fixed when filtering a district.</p></div>
